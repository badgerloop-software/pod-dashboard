...
function clone(id) {
    ...
    if (index === null) {
        ...
        focusBoxes[index] = setInterval(() => {
            ...
            const label = index + 1;
            document.getElementById('header_value_' + label).innerHTML = value;
            ...
            document.getElementById('header_label_' + label).innerHTML = name;
        }, RATE);
    }
}
...
function attachClear() {
    for (let i = 0; i < focusBoxes.length; i++) {
        document.getElementById(`header_focus_${String(i + 1)}`).onclick = function() {
            clearFocusBox(i);
        }
    }
}
